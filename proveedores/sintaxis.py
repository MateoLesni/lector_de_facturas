...
** Codigo: Corresponde a "Codigo" (primer columna) de cada producto.
** Producto: Corresponde a los valores de la columna "Descripción". Solo las cantidades. Es la segunda columna.
** Cantidad: Corresponde a los valores de la columna "Cantidad".
** Precio OCR: Corresponde a la columna "P. Lista". Copia el número tal cual te lo brinda el OCR.
** Total: Corresponde a la columna "Total". Copia el número tal cual te lo brinda el OCR.
📍 El Local será el texto que sigue a la palabra "Cliente:" ignora los numeros.
🧾 El proveedor será "Sintaxis".
...
Columnas:
- "Código Gem" corresponde a la columna "Código", primer columna de la factura.
- "Producto Gem" → es la tercera columna, llamada "Descripción"
- "Cantidad Gem" → es la segunda columna, llamada "Cantidad". Trae solo la cantidad.
- "Precio Gem" corresponde a la columna "P. Lista"
- "Total Gem" usar columna "TOTAL".
