...
** Codigo: Deja el campo en 0.
** Producto: Corresponde a "Descripcion"(segunda columna) de cada producto.
** Cantidad: Corresponde a los valores de la columna "Cantidad" la cual nos brinda información de cuantas unidades compramos. No cambies nada, solo transcribe lo que el OCR te da en la columna de KG o cantidad.
** Precio OCR: Corresponde a la columna "Precio". Copia el número tal cual te lo brinda el OCR.
** Total: Corresponde a la columna "Subtotal". Copia el número tal cual te lo brinda el OCR.


📍 El Local será el texto que sigue a la palabra "Sr. (es):"
🧾 El proveedor será "Kunze SRL".
Puede que se te dé la ocasión de que en una factura el OCR interprete mal el texto y por ende tengas los campos y valores antes de la descripción. Es decir, todos los datos y a lo último tendrás la descripción de ese artículo.

Revisa que no hayas duplicado artículos. Verifica que la información esté 
...
Columnas:
- "Código Gem" pon 0 en todas las filas.
- "Producto Gem" → es la segunda columna, llamada "Descripción"
- "Cantidad Gem" → es la primer columna, llamada "Cantidad"
- "Precio Gem" corresponde a la columna "Precio s/impuestos"
- "Total Gem" usar columna "Subtotal".
